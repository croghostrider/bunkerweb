...
class Test(ABC) :
    ...
    def __init__(self, name, kind, timeout, tests, no_copy_container=False, delay=0):
        ...
        log(
            "TEST",
            "ℹ️",
            f"instiantiated with {len(tests)} tests and timeout of {str(timeout)}s for {self._name}",
        )
        ...
        try:
            ...
            for rm_dir in rm_dirs:
                if isdir(f"/tmp/bw-data/{rm_dir}"):
                    run(f"sudo bash -c 'rm -rf /tmp/bw-data/{rm_dir}/*'", shell=True)
    ...
    def run_tests(self):
        if not self._setup_test() :
            ...
            return False
        if self.__delay != 0:
            log("TEST", "ℹ️", f"delay is set, sleeping {str(self.__delay)}s")
            sleep(self.__delay)
        start = time()
        while time() < start + self._timeout:
            ...
            if all_ok:
                ...
                log("TEST", "ℹ️", f"success ({elapsed}/{str(self._timeout)}s)")
                return self._cleanup_test()
            log("TEST", "⚠️", "tests not ok, retrying in 1s ...")
        self._debug_fail()
        self._cleanup_test()
        log("TEST", "❌", f"failed (timeout = {str(self._timeout)}s)")
        return False
    ...
    def replace_in_file(self, old, new):
        try:
            ...
            log("TEST", "⚠️", f"can't replace file {self} : {format_exc()}")

    def replace_in_files(self, old, new):
