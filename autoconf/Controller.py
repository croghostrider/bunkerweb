...
class Controller(ABC) :
    ...
    def wait(self, wait_time):
        while True:
            ...
            if len(self._instances) == 0:
                log("CONTROLLER", "⚠️", f"No instance found, waiting {str(wait_time)}s ...")
                sleep(wait_time)
                continue
            all_ready = True
    ...
    def get_instances(self):
        ...
        for controller_instance in self._get_controller_instances():
            instances.extend(iter(self._to_instances(controller_instance)))
        return instances
    ...
    def get_services(self):
        ...
        for controller_service in self._get_controller_services():
            services.extend(iter(self._to_services(controller_service)))
        services.extend(iter(self._get_static_services()))
        return services
