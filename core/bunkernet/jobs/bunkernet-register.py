    ...
    if not os.path.isfile("/opt/bunkerweb/cache/bunkernet/instance.id"):
        ...
        if not ok:
            ...
            logger.log(
                "BUNKERNET",
                "❌",
                f"Error {str(status)} from BunkerNet API : " + data["data"],
            )
            os._exit(1)
        elif data["result"] != "ok" :
        ...
        logger.log(
            "BUNKERNET",
            "ℹ️",
            "Successfully registered on BunkerNet API with instance id "
            + bunkernet_id,
        )
    else:
        ...
        logger.log(
            "BUNKERNET",
            "ℹ️",
            f"Already registered on BunkerNet API with instance id {get_id()}",
        )
